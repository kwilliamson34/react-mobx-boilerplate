...
export default class GeolinkControls extends React.Component {
  ...
  onKeyDown = (event) => {
    ...
    if (event.keyCode === this.DOWN_KEY_CODE) {
      this.focusedFavorite = this.store.predictedFavorites.length
        ? (this.focusedFavorite + 1) % (this.store.predictedFavorites.length + 1)
        : 0;
      this[`favItem${this.focusedFavorite}`].focus();
      ...
      if (this.focusedFavorite === 0) {
        ...
        this.focusedFavorite = this.store.predictedFavorites.length
          ? (this.focusedFavorite + this.store.predictedFavorites.length) % (this.store.predictedFavorites.length + 1)
          : 0;
        this[`favItem${this.focusedFavorite}`].focus();
      }
    }
  ...
  renderNetworkSubscriptionLink = () => {
    return (
      <div className="col-xs-12 col-sm-8 col-md-6 col-lg-8 no-gutters">
        <div className="network-subscription-link">
          <Link to="/subscribe-to-alerts">Subscribe to Network Alerts</Link>
        </div>
      </div>
    )
  }
  ...
  renderNetworkLegend = () => {
    return (
      <div className="legend-block">
        <h2>Network<span className="sr-only">&nbsp;color key</span></h2>
  ...
  renderWeatherLegend = () => {
    return (
      <div className="legend-block">
      <h2>Weather<span className="sr-only">&nbsp;color key</span>
      </h2>
      <div className="key-labels">
  ...
  renderWindLegend = () => {
    return (
      <div className="legend-block">
        <h2>Wind<span className="sr-only">&nbsp;color key</span></h2>
        <div className="key-labels">
          <span className="as-label">31-57 MPH</span>
          <span className="as-label">40-58 MPH</span>
          <span className="as-label">74+ MPH</span>
          <span className="as-label">115 MPH</span>
        </div>
        <div className="legend-gradient">
          <img src="/images/wind-legend.png" alt="" />
        </div>
      </div>
    )
  }
  ...
  renderFireLegend = () => {
    return (
      <div className="col-xs-6">
        <h2>Fire<span className="sr-only">&nbsp;color key</span></h2>
        <div className="legend-gradient fire"></div>
      </div>
    )
  }

  renderFlashFloodLegend = () => {
    return (
      <div className="col-xs-6">
        <h2>Flash Flood<span className="sr-only">&nbsp;color key</span></h2>
        <div className="legend-gradient flood"></div>
      </div>
    )
  }
      ...
      <React.Fragment>
        ...
        <div className="traffic-legend-wrapper ">
          ...
          <div className="hazards-legend two-col-legend">
            <span className="col-xs-6 as-label"><i className="icon-warning incident-minor" aria-hidden="true" /><br className="visible-xs" />Minor/Moderate Incident</span>
            <span className="col-xs-6 as-label"><i className="icon-warning incident-serious" aria-hidden="true" /><br className="visible-xs" />Serious Incident</span>
          </div>
        </div>
      </React.Fragment>
    )
    ...
    return (
      ...
      <div className="container">
        <div className="row is-flex">
          <div className="col-xs-12 col-md-6 map-controls">
            {this.renderSearchArea()}
            {this.renderLayersArea()}
            {this.renderContactSubscribeBlock()}
          </div>
          <div className="col-xs-12 col-md-6 map-legends">
            {this.renderNetworkLegend()}
            {this.renderWeatherLegend()}
            {this.renderWindLegend()}
            {this.renderFireFloodBlock()}
