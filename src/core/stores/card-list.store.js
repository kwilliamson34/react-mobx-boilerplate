...
class CardListStore {
    ...
    @action getAdminApps() {
        ...
        return apiService.getAdminApps(userStore.user_token).then(success, fail)
    }
    ...
    @action setCurrentApp(psk) {
        ...
        if (!this.searchResults.length) {
            ...
            return apiService.getAppDetails(psk, userStore.user_token).then(success, failure);
        }
    }
    ...
    @action getAppDetailByPSK(psk) {
        ...
        apiService.getAppDetails(psk, userStore.user_token).then(success, failure);
    ...
    @action changeAppAvailability(appPSK, isAvailable) {
        ...
        if (isAvailable) {
            apiService.addAppToGroup(appPSK, 'Available', userStore.user_token);
        } else {
            apiService.removeAppFromGroup(appPSK, 'Available', userStore.user_token);

            if (this.currentApp.isRecommended) {
                apiService.removeAppFromGroup(appPSK, 'Recommended', userStore.user_token);
            }
        }
    }
    ...
    @action changeAppRecommended(appPSK, isRecommended) {
        ...
        if (this.currentApp.isAvailable) {
            ...
            if (isRecommended) {
                apiService.addAppToGroup(appPSK, 'Recommended', userStore.user_token);
            } else {
                apiService.removeAppFromGroup(appPSK, 'Recommended', userStore.user_token);
            }
        }
