...
describe("UserStore", () => {
  ...
  apiService.validateUserData = jest.fn().mockReturnValue(new Promise(resolve => resolve({
  ...
  describe("logs users out", () => {
    apiService.logoutUser = jest.fn().mockReturnValue(new Promise(resolve => resolve(logoutUserMock())))
    it("successfully", () => {
      store.logoutUser();
